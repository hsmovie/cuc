...
// load components 로고는 헤더의 멍청한 컴포넌트에서 추출해 온것.
...
class App extends Component {
    ...
    handleAuth = async(provider) => {
        ...
        try {
            ...
            if (e.code === 'auth/account-exists-with-different-credential') {
                //계정 링크를 시도한다. 어떤 프로바이더로 가입되어있는지 확인
    ...
    render() {
        ...
        const logoutToggle = this.state.toggle
            ? () => handleModal.open({modalName: 'logout'})
            : () => handleModal.open({modalName: 'login'});
        ...
        return (
            <div>
                ...
                <LinkAccountModal
                    ...
                    onLinkAccount={handleLinkAccount}/> {renderChildren()}
            </div>
        );
    }
}
